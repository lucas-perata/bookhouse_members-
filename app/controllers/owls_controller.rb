class OwlsController < ApplicationController
  ...
  before_action :authenticate_member!, except: [:index, :new]

  # GET /owls or /owls.json
  def index
    @owls = Owl.all.limit(5).order("created_at DESC")
    @owl =Owl.new
  end
  ...
  def create
    @owl = current_member.owls.build(owl_params)
